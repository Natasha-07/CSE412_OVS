	<div class="row">
		...
		<div class="col-sm-12">
			<div class="box box-success box-solid">
				...
				<div class="box-body">
					<button class="btn btn-success btn-sm btn-flat add"><i class="fa fa-plus"></i> Add Admin</button>
					<hr>
	...
	<script>
				...
				success: function(data){
					...
					for (var i = 0; i < data.length; i++) {
						html += '<tr>'+
									...
									'<td><button class="btn btn-xs btn-flat btn-success edit" data="'+data[i].id_admin+'"><i class="fa fa-edit"></i> Edit</button>&nbsp;&nbsp;<button class="btn btn-xs btn-flat btn-danger delete" data="'+data[i].id_admin+'"><i class="fa fa-trash"></i> Delete</button></td>'+
								'</tr>';
					}
		...
		//delete Admin
		$('#data-admin').on('click', '.delete', function(){
			var id = $(this).attr('data');
			var k = confirm('Administrator will be removed ?');
			if (k) {
				$.ajax({
					url: base_url+'admin/delete_admin/'+id,
					success: function(){
	...
	<?=($this->session->flashdata('error_pass')) ? '<script>Swal.fire("Error", "Wrong old password!", "error")</script>' : ''?>